...
class App(Tk):
    ...
    def buildGui(self):
        ...
        def processVideoApply():
            ...
            print('the ox')
            print(self.theox)
            tracker = VehicleCounter(file, video_dim, fps, self.line_coords,
                                     15, showVideo, self.theox, self.theoy, self.thedx, self.thedy)
            tracker.start()
        ...
        def buildThreshold(img, thickness, origin_coords, destination_coords):
            ...
            len = sqrt((destination_coords[0] - origin_coords[0])
                       ** 2 + (destination_coords[1] - origin_coords[1])**2)
            (udx, udy) = (dx / len, dy / len)
            (px, py) = (-udy, udx)
            (x1p, y1p) = (
                origin_coords[0] + px * cleanThreshold, origin_coords[1] + py * cleanThreshold)
            (x2p, y2p) = (x1p + dx, y1p + dy)
            (x3p, y3p) = (
                origin_coords[0] - px * cleanThreshold, origin_coords[1] - py * cleanThreshold)
            (x4p, y4p) = (x3p + dx, y3p + dy)

            self.theox = origin_coords[0]
            self.theoy = origin_coords[1]
            self.thedx = destination_coords[0]
            self.thedy = destination_coords[1]
            (pt1x, pt1y) = (origin_coords[0], origin_coords[1])
            (pt2x, pt2y) = (destination_coords[0], origin_coords[1])
            ...
            # box corner approach
            edited_imaged = cv2.line(img=img, pt1=(floor(pt1x), floor(pt1y)), pt2=(
                floor(pt2x), floor(pt2y)), color=(0, 255, 0), thickness=thickness)
            edited_imaged = cv2.line(img=img, pt1=(floor(pt2x), floor(pt2y)), pt2=(
                floor(pt3x), floor(pt3y)), color=(0, 255, 0), thickness=thickness)
            edited_imaged = cv2.line(img=img, pt1=(floor(pt3x), floor(pt3y)), pt2=(
                floor(pt4x), floor(pt4y)), color=(0, 255, 0), thickness=thickness)
            edited_imaged = cv2.line(img=img, pt1=(floor(pt4x), floor(pt4y)), pt2=(
                floor(pt1x), floor(pt1y)), color=(0, 255, 0), thickness=thickness)
        ...
        def get_click_coords(event, raw_image):
            ...
            if (self.clicked_coords == (0, 0)):
                ...
                edited_imaged = cv2.putText(img=edited_imaged, text=origin_coord, org=(
                    0, 50), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=(255, 0, 0))
                edited_imaged = cv2.putText(img=edited_imaged, text=" -> ", org=(
                    200, 50), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=(255, 0, 0))
                ...
                edited_imaged = cv2.putText(img=edited_imaged, text=" -> ", org=(
                    200, 50), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=(255, 0, 0))
                edited_imaged = cv2.putText(img=edited_imaged, text=destination_coord, org=(
                    300, 50), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=(255, 0, 0))
                dest_coord_val.delete(0, END)
                dest_coord_val.insert(0, f'{destination_coord}')
                edited_imaged = cv2.line(img=edited_imaged, pt1=self.clicked_coords, pt2=(
                    event.x, event.y), color=(0, 255, 0), thickness=3)
                edited_imaged = buildThreshold(
                    edited_imaged, 1, self.clicked_coords, (event.x, event.y))
                edited_imaged = Image.fromarray(edited_imaged)
                imgtk = ImageTk.PhotoImage(image=edited_imaged)
                lmain.imgtk = imgtk
                lmain.configure(image=imgtk)
        ...
        def video_apply():
            ...
            if success:
                ...
                lmain.imgtk = imgtk
                lmain.configure(image=imgtk)
                lmain.bind("<Button>", lambda evt: get_click_coords(
                    evt, self.raw_image))
            cap.release()
        ...
        instructions = Label(master=inputs_frame,
                             text="Drag an mp4 or .avi onto the bar")
        ...
        input_video_val = Entry(master=inputs_frame,
                                textvar=strVar, bd=5, w="70")
        # input_video_val.drop_target_register(DND_FILES)
        apply_video_btn = Button(
            master=inputs_frame, text=u"\U0001F4C1", command=video_apply)
        ...
        process_button = Button(
            footer, text="Process Video", command=processVideoApply)
